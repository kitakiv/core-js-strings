...
function sumOfCodes(str) {
  if (str) {
    ...
    return str.split('').reduce((acc, elem) => (acc += elem.charCodeAt()), 0);
  }
  return 0;
}
...
function countVowels(str) {
  const volve = ['a', 'e', 'i', 'o', 'u', 'y', 'A', 'E', 'I', 'O', 'U', 'Y'];
  return (
    str
      .split('')
      // eslint-disable-next-line no-return-assign, no-param-reassign
      .reduce((acc, elem) => (volve.includes(elem) ? (acc += 1) : acc), 0)
  );
}
...
function isPalindrome(str) {
  let str1 = str.includes(',') ? str.replaceAll(',', '') : str;
  str1 = str1.includes('!') ? str1.replaceAll('!', '') : str1;
  str1 = str1.includes('?') ? str1.replaceAll('?', '') : str1;
  str1 = str1.includes(' ') ? str1.replaceAll(' ', '') : str1;
  return str1.split('').reverse().join('').toLowerCase() === str1.toLowerCase();
}
...
function findLongestWord(sentence) {
  return sentence.split(' ').reduce((acc, elem) => {
...
function reverseWords(str) {
  return str
    .split(' ')
    .reduce((acc, elem) => [...acc, elem.split('').reverse().join('')], [])
    .join(' ');
}
